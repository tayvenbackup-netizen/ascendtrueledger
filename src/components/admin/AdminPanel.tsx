...
import { X, Plus, Copy, Trash2, RefreshCw, Shield, Loader2, KeyRound } from 'lucide-react';
...
const AdminPanel = ({ open, onClose }: Props) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api('list_keys');
      if (Array.isArray(data?.keys)) setKeys(data.keys);
      else if (data?.error) { setAuthed(false); setAuthError(data.error); }
    } catch {}
    setLoading(false);
  }, []);
  ...
  const doAuth = async (e: React.FormEvent) => {
    ...
    try {
      const data = await api('admin_auth', { admin_password: pw });
      if (data?.success) {
  ...
  const create = async () => {
    setCreating(true);
    try {
      const data = await api('generate_key', { key_type: newType });
      if (data?.key) { setLastKey(data.key); await load(); }
  ...
  const revoke = async (id: string) => {
    ...
    await api('revoke_key', { key_id: id });
    load();
  };
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <button onClick={close} className="text-white/70"><X className="w-5 h-5" /></button>
            </div>

            {!authed && (
              <form onSubmit={doAuth} className="p-5 space-y-3">
                <p className="text-white/60 text-sm">Enter the master admin password.</p>
                <div className="relative">
                  <KeyRound className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                  <input type="password" value={pw} onChange={(e) => setPw(e.target.value)} autoFocus
                    placeholder="Admin password"
                    className="w-full h-11 pl-10 pr-3 rounded-lg text-sm text-white"
                    style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(187,174,252,0.2)' }} />
                </div>
                {authError && <div className="text-xs" style={{ color: '#ff7a7a' }}>{authError}</div>}
                <button type="submit" disabled={authing}
                  className="w-full h-11 rounded-lg text-white font-semibold text-sm flex items-center justify-center gap-2"
                  style={{ background: 'linear-gradient(135deg,#6c5ce7,#8b6cf3)' }}>
                  {authing ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Unlock Admin'}
                </button>
              </form>
            )}
            ...
            {authed && (
              <>
                ...
                {lastKey && (
                  <div className="px-5 py-3 flex items-center gap-2" style={{ background: 'rgba(187,174,252,0.08)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    <span className="text-xs text-white/60">New:</span>
                    <code className="text-sm font-mono text-white flex-1 truncate">{lastKey}</code>
                    <button onClick={() => copy(lastKey)} className="text-white/70"><Copy className="w-3.5 h-3.5" /></button>
                  </div>
                )}
                    ...
                    <div key={k.id} className="px-5 py-3 flex items-center gap-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-white/50 mt-0.5">
                          {k.key_type} · {k.is_revoked ? 'revoked' : 'active'}
                          {k.expires_at && ` · exp ${new Date(k.expires_at).toLocaleDateString()}`}
                        </div>
                      </div>
                      {!k.is_revoked && (
